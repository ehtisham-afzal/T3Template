...
export default function SessionData({ session }: { session: Session | null }) {
  if (session?.user) {
    return (
      <div className="flex flex-col gap-4 p-4 w-full bg-secondary rounded-md">
        ...
        <div className="flex flex-col rounded-md bg-muted-foreground text-secondary">
          <div className="p-4 font-bold rounded-t-md  text-secondary bg-muted-foreground/60">
            Session
          </div>
          <pre className="py-6 px-4 whitespace-pre-wrap break-all text-secondary">
  ...
  return (
    <p className="p-4 w-full  rounded-md text-secondary bg-primary">
